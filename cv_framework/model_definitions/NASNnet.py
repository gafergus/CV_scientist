...
def reduction_a_cell(ip, p, filters, block_id=None):
    ...
    with keras.backend.name_scope('reduction_A_block_{}'.format(block_id)):
        p = adjust_block(p, ip, filters, block_id)

        h = keras.layers.Activation('relu')(ip)
        h = keras.layers.Conv2D(filters, (1, 1), strides=(1, 1), padding='same', name='reduction_conv_1_{}'.format(block_id),
                                use_bias=False, kernel_initializer='he_normal')(h)
        h = keras.layers.BatchNormalization(axis=3, momentum=0.9997, epsilon=1e-3, name='reduction_bn_1_{}'.format(block_id))(h)
        h3 = keras.layers.ZeroPadding2D(padding=correct_pad(h, 3), name='reduction_pad_1_{}'.format(block_id))(h)
        ...
        with keras.backend.name_scope('block_1'):
            ...
            x1 = keras.layers.add([x1_1, x1_2], name='reduction_add_1_{}'.format(block_id))
        ...
        with keras.backend.name_scope('block_2'):
            ...
            x2 = keras.layers.add([x2_1, x2_2], name='reduction_add_2_{}'.format(block_id))
        ...
        with keras.backend.name_scope('block_3'):
            ...
            x3 = keras.layers.add([x3_1, x3_2], name='reduction_add3_{}'.format(block_id))
        ...
        with keras.backend.name_scope('block_5'):
            ...
            x5 = keras.layers.add([x5_1, x5_2], name='reduction_add4_{}'.format(block_id))

        x = keras.layers.concatenate([x2, x3, x4, x5], axis=3, name='reduction_concat_{}'.format(block_id))
        return x, ip

def separable_conv_block(ip, filters, kernel_size=(3, 3), strides=(1, 1), block_id=None):
    with keras.backend.name_scope('separable_conv_block_{}'.format(block_id)):
        ...
        if strides == (2, 2):
            ...
            conv_pad = 'same'
        x = keras.layers.SeparableConv2D(filters, kernel_size,strides=strides, name='separable_conv_1_{}'.format(block_id),
                                         padding=conv_pad, use_bias=False, depthwise_initializer='he_normal',
                                         pointwise_initializer='he_normal')(x)
        x = keras.layers.BatchNormalization(momentum=0.9997, epsilon=1e-3, name='separable_conv_1_bn_{}'.format(block_id))(x)
        x = keras.layers.Activation('relu')(x)
        x = keras.layers.SeparableConv2D(filters, kernel_size,name='separable_conv_2_{}'.format(block_id), padding='same',
                                         use_bias=False, depthwise_initializer='he_normal',
                                         pointwise_initializer='he_normal')(x)
        x = keras.layers.BatchNormalization(momentum=0.9997, epsilon=1e-3, name='separable_conv_2_bn_{}'.format(block_id))(x)
    return x
...
def adjust_block(p, ip, filters, block_id=None):
    ...
    with keras.backend.name_scope('adjust_block'):
        if p is None:
            ...
            with keras.backend.name_scope('adjust_reduction_block_{}'.format(block_id)):
                p = keras.layers.Activation('relu', name='adjust_relu_1_{}'.format(block_id))(p)
                ...
                p2 = keras.layers.AveragePooling2D((1, 1), strides=(2, 2), padding='valid',
                                                   name='adjust_avg_pool_2_{}'.format(block_id))(p2)
                p2 = keras.layers.Conv2D(filters // 2, (1, 1),padding='same', use_bias=False,
                                         name='adjust_conv_2_{}'.format(block_id), kernel_initializer='he_normal')(p2)

                p = keras.layers.concatenate([p1, p2])
                p = keras.layers.BatchNormalization(momentum=0.9997, epsilon=1e-3, name='adjust_bn_{}'.format(block_id))(p)

        elif p_shape[-1] != filters:
            with keras.backend.name_scope('adjust_projection_block_{}'.format(block_id)):
...
def normal_a_cell(ip, p, filters, block_id=None):
    ...
    with keras.backend.name_scope('normal_A_block_{}'.format(block_id)):
        p = adjust_block(p, ip, filters, block_id)

        h = keras.layers.Activation('relu')(ip)
        h = keras.layers.Conv2D(filters, (1, 1), strides=(1, 1), padding='same', name='normal_conv_1_{}'.format(block_id),
                                use_bias=False, kernel_initializer='he_normal')(h)
        h = keras.layers.BatchNormalization(axis=3, momentum=0.9997, epsilon=1e-3, name='normal_bn_1_{}'.format(block_id))(h)
        ...
        with keras.backend.name_scope('block_1'):
            ...
            x1 = keras.layers.add([x1_1, x1_2], name='normal_add_1_{}'.format(block_id))
        ...
        with keras.backend.name_scope('block_2'):
            ...
            x2 = keras.layers.add([x2_1, x2_2], name='normal_add_2_{}'.format(block_id))

        with keras.backend.name_scope('block_3'):
            x3 = keras.layers.AveragePooling2D((3, 3), strides=(1, 1), padding='same',
                                               name='normal_left3_{}'.format(block_id))(h)
            x3 = keras.layers.add([x3, p], name='normal_add_3_{}'.format(block_id))
        ...
        with keras.backend.name_scope('block_4'):
            ...
            x4 = keras.layers.add([x4_1, x4_2], name='normal_add_4_{}'.format(block_id))

        with keras.backend.name_scope('block_5'):
            x5 = separable_conv_block(h, filters, block_id='normal_left5_{}'.format(block_id))
            x5 = keras.layers.add([x5, h], name='normal_add_5_{}'.format(block_id))

        x = keras.layers.concatenate([p, x1, x2, x3, x4, x5], axis=3, name='normal_concat_{}'.format(block_id))
    return x, ip
